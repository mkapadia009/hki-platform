...
export default function NewCustomerIncidentPage() {
  ...
  useEffect(() => {
    // This will fetch only the customer's orders due to backend filtering
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend automatically sets customerId from the JWT
      const res = await api.post('/incidents', payload);
      addToast('Success: Your ticket has been logged and the support team has been notified.', 'success');
      router.push(`/customer-portal/incidents/${res.data.id}`);
    } catch (error) {
      console.error(error);
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Raise New Incident</h1>
          <p className="text-sm text-gray-500">Log a support ticket. If this is about a specific product you purchased, please select it below.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg space-y-4 border border-blue-100 dark:border-blue-800/50">
            <h3 className="font-medium text-blue-800 dark:text-blue-300">Link to a Purchased Product (Optional)</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1 text-blue-900 dark:text-blue-200">Select Order</label>
                <select 
                ...
                >
                  ...
                  {orders.map(o => <option key={o.id} value={o.id}>{o.orderNumber} - {new Date(o.orderDate).toLocaleDateString()}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-blue-900 dark:text-blue-200">Select Product</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-blue-900 dark:text-blue-200">Select AMC</label>
                <select 
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t flex justify-end gap-3">
          ...
          <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
            <Save size={18} /> {loading ? 'Submitting...' : 'Submit Incident'}
          </button>
        </div>
      </form>
    </div>
